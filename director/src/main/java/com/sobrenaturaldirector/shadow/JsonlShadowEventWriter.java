...
public final class JsonlShadowEventWriter implements ShadowEventSink {
    ...
    public synchronized void accept(ShadowScoreResult r){String line=serialize(r);long bytes=line.getBytes(java.nio.charset.Charset.forName("UTF-8")).length;if(total+bytes>totalLimit)throw new IllegalStateException("shadow disk budget reached");try{if(writer==null||segment+bytes>segmentLimit)rotate();writer.write(line);writer.newLine();writer.flush();segment+=bytes;total+=bytes;}catch(IOException e){throw new IllegalStateException("shadow writer failure",e);}}
    ...
    private String serialize(ShadowScoreResult r){StringBuilder b=new StringBuilder();b.append("{\"eventType\":\"SHADOW_SCORE_RESULT\",\"schema\":\"").append(esc(r.getSnapshot().getSchema())).append("\",\"decisionId\":\"").append(esc(r.getSnapshot().getDecisionId())).append("\",\"tick\":").append(r.getSnapshot().getTick()).append(",\"heuristicCandidateId\":\"").append(esc(r.getSnapshot().getHeuristicCandidateId())).append("\",\"shadowSelectedId\":\"").append(esc(r.getShadowSelectedId())).append("\",\"latencyNanos\":").append(r.getLatencyNanos()).append(",\"scores\":{");boolean first=true;for(java.util.Map.Entry<String,Double> e:r.getScores().entrySet()){if(!first)b.append(',');first=false;b.append('\"').append(esc(e.getKey())).append("\":").append(e.getValue());}return b.append("}}").toString();}
