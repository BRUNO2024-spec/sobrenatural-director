...
public final class ShadowObservationService {
    ...
    public ShadowObservationService(ShadowScorer scorer,ShadowEventSink sink,int capacity,com.sobrenaturaldirector.research.ResearchCollectionGate gate){if(capacity<1||scorer==null||sink==null)throw new IllegalArgumentException("invalid shadow service");this.scorer=scorer;this.sink=sink;this.collectionGate=gate;this.queue=new ArrayBlockingQueue<ShadowDecisionSnapshot>(capacity);}
    ...
    public boolean submit(ShadowDecisionSnapshot snapshot){if(snapshot==null||!enabled||stopping||collectionGate!=null&&!collectionGate.canCollect())return false;if(!queue.offer(snapshot)){metrics.dropped();return false;}metrics.submitted();return true;}
