...
public final class ShadowObservationService {
    ...
    private final com.sobrenaturaldirector.research.ResearchCollectionGate collectionGate;
    private final ShadowObservationMetrics metrics=new ShadowObservationMetrics();
    private volatile boolean enabled,stopping;
    private Thread worker;
    public ShadowObservationService(ShadowScorer scorer,ShadowEventSink sink,int capacity){this(scorer,sink,capacity,null);}
    public ShadowObservationService(ShadowScorer scorer,ShadowEventSink sink,int capacity,com.sobrenaturaldirector.research.ResearchCollectionGate gate){if(capacity<1||scorer==null||sink==null)throw new IllegalArgumentException("invalid shadow service");this.scorer=scorer;this.sink=sink;this.collectionGate=gate;this.queue=new ArrayBlockingQueue<ShadowDecisionSnapshot>(capacity);if(sink instanceof JsonlShadowV2EventWriter)((JsonlShadowV2EventWriter)sink).bindMetrics(metrics);}
    public synchronized void start(){if(enabled)return;enabled=true;stopping=false;if(sink instanceof JsonlShadowV2EventWriter)((JsonlShadowV2EventWriter)sink).healthSnapshot(metrics,0,queue.size());worker=new Thread(new Runnable(){public void run(){loop();}},"director-shadow-worker");worker.setDaemon(true);worker.start();}
