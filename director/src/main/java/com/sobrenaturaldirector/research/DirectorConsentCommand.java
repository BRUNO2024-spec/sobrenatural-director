...
public final class DirectorConsentCommand extends CommandBase {
    ...
    public void processCommand(ICommandSender sender,String[] args){String key=sender.getCommandSenderName();if(args.length<2||!"consent".equalsIgnoreCase(args[0])){sender.addChatMessage(new net.minecraft.util.ChatComponentText(getCommandUsage(sender)));return;}String op=args[1].toLowerCase();if("accept".equals(op)){sender.addChatMessage(new net.minecraft.util.ChatComponentText("Research consent accepted for this session; participant="+registry.accept(key)));}else if("decline".equals(op)){registry.decline(key);sender.addChatMessage(new net.minecraft.util.ChatComponentText("Research consent declined; no research collection for this session."));}else if("revoke".equals(op)){registry.revoke(key);sender.addChatMessage(new net.minecraft.util.ChatComponentText("Research consent revoked; new research records are disabled."));}else if("status".equals(op)){sender.addChatMessage(new net.minecraft.util.ChatComponentText("Research consent="+registry.state(key)));}else sender.addChatMessage(new net.minecraft.util.ChatComponentText(getCommandUsage(sender)));}
