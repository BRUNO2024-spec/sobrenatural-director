...
public final class NarrativeActionBridge {
    ...
    public ActionPlan composePlan(SituationBlueprint blueprint, DimensionRef dimension, String planId, SemanticActionCatalog catalog) {
        if (blueprint == null || dimension == null || planId == null || catalog == null) throw new IllegalArgumentException("blueprint plan inputs");
        return new ActionComposer().compose(planId, fromBlueprint(blueprint, dimension).getType(), dimension, catalog);
    }
}
