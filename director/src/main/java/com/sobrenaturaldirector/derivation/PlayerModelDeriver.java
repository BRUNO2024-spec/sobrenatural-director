...
public final class PlayerModelDeriver {
    public PlayerDerivation derive(PlayerSnapshot p,boolean partial){List<DerivedSignal>s=new ArrayList<DerivedSignal>();List<String>w=new ArrayList<String>();
        ...
        if(partial)w.add("DERIVATION_INPUT_PARTIAL"); boolean underground=!p.getEnvironment().canSeeSky()&&p.getEnvironment().getY()<62; PlayerModel model=new PlayerModel(p.getPlayerId(),p.getDimension(),s.get(0),band,s.get(1),s.get(2),s.get(3),s.get(4),s.get(10),s.get(5),s.get(6),s.get(7),s.get(8),s.get(9),occupied,128-occupied,stacks,armorSlots,damaged,p.isAlive(),partial?Confidence.PARTIAL:Confidence.KNOWN,underground);return new PlayerDerivation(model,new DerivationTrace(s,w)); }
