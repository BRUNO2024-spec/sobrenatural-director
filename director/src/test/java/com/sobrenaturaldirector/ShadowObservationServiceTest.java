...
public class ShadowObservationServiceTest {
    ...
    @Test public void queueOverflowDropsWithoutBlocking() throws Exception {
        ...
        ShadowObservationService service=new ShadowObservationService(scorer,new ShadowEventSink(){public void accept(ShadowScoreResult r){}public void close(){}},1);service.start();service.submit(snapshot("a"));assertTrue(service.getMetrics().getHighWatermark()>=1);assertTrue(scoring.await(2,TimeUnit.SECONDS));service.submit(snapshot("b"));long start=System.nanoTime();service.submit(snapshot("c"));long elapsed=System.nanoTime()-start;assertTrue(elapsed<100000000L);assertTrue(service.getMetrics().getDropped()>=1);release.countDown();service.stop();
    }
