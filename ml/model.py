...
class LearnedScorerV1(nn.Module):
    def __init__(self, numeric_dim, vocab_sizes, embedding_dim=4, hidden=32):
        super().__init__()
        self.embeddings = nn.ModuleList([nn.Embedding(n, embedding_dim) for n in vocab_sizes])
        self.net = nn.Sequential(nn.Linear(numeric_dim + embedding_dim*len(vocab_sizes), hidden), nn.ReLU(), nn.Linear(hidden, 1))
