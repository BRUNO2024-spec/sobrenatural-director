...
def main():
 ...
 result={'counts':counts,'total_states':len(states),'total_rows':sum(x['rows'] for x in counts.values()),'duplicate_or_leakage_errors':bad,'shard_count':len(shards),'shards':shards,'status':'PASS' if not bad else 'FAIL'};json.dump(result,open(a.out,'w'),indent=2);print(json.dumps({k:result[k] for k in ('counts','total_states','total_rows','shard_count','status')}))
