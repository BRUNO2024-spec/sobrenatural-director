...
def main():
    ...
    manifest={"datasetName":"DIRECTOR_EXPERIENCE_DATASET_V3","datasetVersion":"DIRECTOR_EXPERIENCE_DATASET_V3","runId":args.run_id,"sourceCommit":source_commit,"featureSchema":SCHEMA,"generatorVersion":VERSION,"globalSeed":args.seed,"totalStateGroups":args.states,"totalCandidateRows":sum(x["rows"] for x in counts.values()),"splits":counts,"sequenceEpisodes":episodes,"sequenceStates":min(args.states,episodes*ep_steps),"teacherProvenance":"DETERMINISTIC_BENCHMARK_TEACHER","teacherImplementation":"ml/generate_experience_v3.py::quality","teacherFingerprint":teacher_fingerprint,"featureSchemaSha256":sha_obj(schema),"generationSpecSha256":hashlib.sha256((config_path.read_bytes() if config_path.exists() else json.dumps(spec,sort_keys=True,separators=(",",":")).encode())).hexdigest(),"shardCount":len(shards),"shards":shards,"totalBytes":sum(x["bytes"] for x in shards),"createdUtc":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime())}
