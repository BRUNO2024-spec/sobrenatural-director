...
def main():
 ...
 c=x['config'];m=LearnedScorerV2(d['numeric_dim'],d['vocab_sizes'],hidden=(64,),dropout=0.0);m.load_state_dict(x['model_state_dict']);o=torch.optim.Adam(m.parameters(),lr=c.get('lr',.001));
 if x.get('optimizer_state_dict'): o.load_state_dict(x['optimizer_state_dict'])
