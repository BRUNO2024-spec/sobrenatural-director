...
def main():
 ...
 for epoch in range(a.epochs):
  ...
  m.eval();
  with torch.no_grad(): pred=m(vn.to(device),vc.to(device)).cpu().numpy()
  mm=agg(pred,vy.numpy(),vg.numpy());hist.append({'epoch':epoch+1,'validation':mm,'train_loss':total});print(json.dumps(hist[-1]),flush=True)
