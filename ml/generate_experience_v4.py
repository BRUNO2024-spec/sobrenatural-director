...
def main():
 ...
 for i in range(a.states):
  ep=i//10 if i<20000 else None;step=i%10 if ep is not None else 0;s=state(i,a.seed,ep,step);sp='VALIDATION_STRESS' if i%10==9 else 'VALIDATION_IID' if i%10==8 else 'TRAIN';b=bucket(i);cs=candidates(s);chosen=max(((quality(s,c),c) for c in cs),key=lambda z:(z[0],z[1]))[1];counts[sp]['states']+=1
  for c in cs:
